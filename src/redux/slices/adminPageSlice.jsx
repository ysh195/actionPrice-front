...
export const blockUser = createAsyncThunk(
    ...
    async (username) => {
        ...
        return { username, message: response.data.message, isBlocked: response.data.isBlocked };
});
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchUserList.fulfilled, (state, action) => {
        const { userList, currentPageNum, currentPageSize, listSize, totalPageNum, hasNext, keyword } = action.payload;
      ...
      .addCase(blockUser.fulfilled, (state, action) => {
        const user = state.userList.find(user => user.username === action.payload.username);
        if (user) {
